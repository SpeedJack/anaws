 ...
 * {@link DualAscentTopologyManager}
 */
public class DualAscentTopologyInstance implements ITopologyInstance
{
	...
	private static final Logger log = LoggerFactory.getLogger(DualAscentTopologyInstance.class);
	...
	private Map<DatapathId, Set<NodePortTuple>> portsBroadcastPerArchipelago; /*
											 * broadcast ports in each
											 * archipelago ID
											 */
//------------------------------------------------------------------------------
	private Map<Link, Integer> reducedCost = new HashMap<>();
	...
	private Comparator<Link> comparatorLinks = new Comparator<Link>() {
		...
		{
			Integer costLinkLeft = reducedCost.get(linkLeft);
			Integer costLinkRight = reducedCost.get(linkRight);
			if (costLinkLeft == costLinkRight)
				return 0;
			if (costLinkLeft == null)
				return -1;
			else if (costLinkRight == null)
				return 1;
			else
...
//------------------------------------------------------------------------------
	...
	protected DualAscentTopologyInstance(Map<DatapathId, Set<OFPort>> portsWithLinks, Set<NodePortTuple> portsBlocked,
		Map<NodePortTuple, Set<Link>> linksNonBcastNonTunnel, Set<NodePortTuple> portsWithMoreThanTwoLinks,
		Set<NodePortTuple> portsTunnel, Map<NodePortTuple, Set<Link>> links,
		Map<DatapathId, Set<OFPort>> portsPerSwitch, Map<NodePortTuple, Set<Link>> linksExternal)
	{
	...
	{
		/*
		 * Step 1: Compute clusters ignoring ports with > 2 links and blocked links.
		 */
		identifyClusters();

		/*
		 * Step 2: Associate non-blocked links within clusters to the cluster in which
		 * they reside. The remaining links are inter-cluster links.
		 */
		identifyIntraClusterLinks();

		/*
		 * Step 3: Compute the archipelagos. (Def: group of conneccted clusters) Each
		 * archipelago will have its own broadcast tree, chosen by running dijkstra's
		 * algorithm from the archipelago ID switch (lowest switch DPID). We need a
		 * broadcast tree per archipelago since each archipelago is by definition
		 * isolated from all other archipelagos.
		 */
		identifyArchipelagos();

		/*
		 * Step 4: Use Yens algorithm to permute through each node combination within
		 * each archipelago and compute multiple paths. The shortest path located (i.e.
		 * first run of dijkstra's algorithm) will be used as the broadcast tree for the
		 * archipelago.
		 */
		computeOrderedPaths();

		/*
		 * Step 5: Determine the broadcast ports for each archipelago. These are the
		 * ports that reside on the broadcast tree computed and saved when performing
		 * path-finding. These are saved into multiple data structures to aid in quick
		 * lookup per archipelago, per-switch, and topology-global.
		 */
		computeBroadcastPortsPerArchipelago();

		/*
		 * Step 6: Optionally, print topology to log for added verbosity or when
		 * debugging.
		 */
		printTopology();
	...
	private void identifyIntraClusterLinks()
	{
		for (DatapathId s : switches) {
			...
			for (OFPort p : portsWithLinks.get(s)) {
				...
				if (linksNonBcastNonTunnel.get(np) == null)
					continue;
				if (isBroadcastPort(np))
					continue;
				for (Link l : linksNonBcastNonTunnel.get(np)) {
					if (isBlockedLink(l))
						continue;
					if (isBroadcastLink(l))
						continue;
		...
		if (portsWithLinks.get(currSw) != null) {
			for (OFPort p : portsWithLinks.get(currSw)) {
				...
				for (Link l : lset) {
					...
					if (dstSw.equals(currSw))
						continue;

					// ignore if the destination is already added to
					// another cluster
					if (clusterFromSwitch.get(dstSw) != null)
						continue;
	...
	private class NodeDist implements Comparable<NodeDist>
	{
		...
		private DualAscentTopologyInstance getOuterType()
		{
			return DualAscentTopologyInstance.this;
		}
	}
...
//------------------------------------------------------------------------------
	...
	{
		if (costCsv != null && !costCsv.isEmpty())
			log.info("Cost CSV:\nsrc,dst,isDstRooted,dualAscentCost,dijkstraCost\n{}", costCsv);
	}

	private boolean areConnected(DatapathId node1, DatapathId node2)
	{
		if (!connections.containsKey(node1) ||
			!connections.get(node1).containsKey(node2))
			return false;
	...
	{
		HashMap<DatapathId, Link> linksGraphA = new HashMap<>();
		HashMap<DatapathId, Integer> costsNodesGraphA = new HashMap<>();
		Integer costLink;
		HashSet<DatapathId> seenNodes = new HashSet<>();
		PriorityQueue<NodeDist> nodeq = new PriorityQueue<NodeDist>();

		for (DatapathId node: auxiliaryGraph.getNodes()) {
			linksGraphA.put(node, null);
			costsNodesGraphA.put(node, MAX_PATH_WEIGHT);
		}

		nodeq.add(new NodeDist(auxiliaryGraph.getRoot(), 0));
		costsNodesGraphA.put(auxiliaryGraph.getRoot(), 0);
		while (nodeq.peek() != null) {
			...
			for (Link link: auxiliaryGraph.getLinks(node)) {
				DatapathId neighbor = isDstRooted ?
					link.getSrc() : link.getDst();
				if (neighbor.equals(node) ||
					seenNodes.contains(neighbor))
					continue;

				costLink = linkCost.get(link);
				int ndist = cdist + costLink;
				if (ndist < costsNodesGraphA.get(neighbor)) {
					costsNodesGraphA.put(neighbor, ndist);
					linksGraphA.put(neighbor, link);
		...
		BroadcastTree broadcastTree = new BroadcastTree(linksGraphA, costsNodesGraphA);
	...
	private void editCosts(Set<DatapathId> rootComponent, Link minLink,
		Graph completeGraph)
	{
		...
		for (DatapathId node: rootComponent) {
			...
			for (Link ingressLink: nodeLinks) {
				if (!node.equals(ingressLink.getDst()) ||
					rootComponent.contains(ingressLink.getSrc()))
					continue;
	...
	private Link findMinArc(Set<DatapathId> rootComponent,
		Graph completeGraph, Graph auxiliaryGraph)
	{
		Link minLink = null;
		for (DatapathId node: rootComponent)
			for (Link link: completeGraph.getLinks(node)) {
				if (!node.equals(link.getDst()) ||
					auxiliaryGraph.hasLink(node, link))
					continue;
				if (minLink == null ||
					comparatorLinks.compare(minLink, link) > 0)
					minLink = link;
			}
		return minLink;
	}
	...
	private HashSet<DatapathId> findRootComp(Graph completeGraph, Graph auxiliaryGraph)
	{
		...
		externCycle: for (DatapathId node: auxiliaryGraph.getTargets()) {
			rootComponent.clear();
			if (node.equals(root) || areConnected(root, node))
				continue;
			...
			for (DatapathId auxiliaryNode: auxiliaryGraph.getNodes()) {
				if (auxiliaryNode.equals(root) ||
					rootComponent.contains(auxiliaryNode))
					continue;
...
//------------------------------------------------------------------------------
	...
	/*
	 * Calculates and stores n possible paths using Yen's algorithm, looping through
	 * every switch. These lists of routes are stored in the pathcache.
	 */
	...
	private Path buildPath(PathId id, BroadcastTree tree)
	{
		...
		if (tree == null)
			return new Path(id, ImmutableList.of()); /* empty route */
	...
	private Map<DatapathId, Set<Link>> buildLinkDpidMap(Set<DatapathId> switches,
		Map<DatapathId, Set<OFPort>> portsWithLinks, Map<NodePortTuple, Set<Link>> links)
	{
	...
	/**
	 *
	 * This function returns K number of routes between a source and destination IF
	 * THEY EXIST IN THE ROUTECACHE. If the user requests more routes than
	 * available, only the routes already stored in memory will be returned. This
	 * value can be adjusted in floodlightdefault.properties.
	 *
	 *
	...
	public List<Path> getPathsFast(DatapathId src, DatapathId dst, int k)
	{
		...
		if (k >= DualAscentTopologyManager.getMaxPathsToComputeInternal() || k >= routes.size()) {
			return routes;
		} else {
	...
	/**
	 *
	 * This function returns K number of paths between a source and destination. It
	 * will attempt to retrieve these paths from the pathcache. If the user requests
	 * more paths than are stored, Yen's algorithm will be run using the K value
	 * passed in.
	 *
	 *
	...
	public List<Path> getPathsSlow(DatapathId src, DatapathId dst, int k)
	{
		...
		if (k >= DualAscentTopologyManager.getMaxPathsToComputeInternal() || k >= paths.size()) {
	...
	private List<Path> yens(DatapathId src, DatapathId dst, Integer K, Archipelago aSrc, Archipelago aDst)
	{
		...
		// A is the list of shortest paths. The number in the list at the end should be
		...
		/*
		 * Use DualAscent to find the shortest path, which will also be the first path
		 * in A
		 */
		...
		for (int k = 1; k < K; k++) {
			...
			if (log.isTraceEnabled()) {
				log.trace("Path Length 'A.get(k-1).getPath().size()-2': {}",
					A.get(k - 1).getPath().size() - 2);
			}
			// Iterate through i, which is the number of links in the most recent path added
			// to A
			for (int i = 0; i <= A.get(k - 1).getPath().size() - 2; i = i + 2) {
				log.trace("i: {}", i);
				List<NodePortTuple> path = A.get(k - 1).getPath();
				// log.debug("A(k-1): {}", A.get(k - 1).getPath());
				// The spur node is the point in the topology where Dijkstra's is called again
				// to find another path
				DatapathId spurNode = path.get(i).getNodeId();
				// rootPath is the path along the previous shortest path that is before the spur
				// node
				Path rootPath = new Path(new PathId(path.get(0).getNodeId(), path.get(i).getNodeId()),
					path.subList(0, i));

				Map<NodePortTuple, Set<Link>> allLinksCopy = new HashMap<NodePortTuple, Set<Link>>(
					links);
				...
				// Removes the root path so Dijkstra's doesn't try to go through it to find a
				// path
				...
				// Uses DualAscent to try to find a shortest path from the spur node to the
				// destination
			...
			// If we get out of the loop and there isn't a path in B to add to A, all
			// possible paths have been
			// found and return A
			if (B.isEmpty()) {
	...
	private Path removeShortestPath(List<Path> routes, Map<Link, Integer> linkCost)
	{
		...
		for (Path r : routes) {
			...
			// If it is smaller than the current smallest, replace variables with the path
			// just found
	...
	/**
	 * Computes end-to-end path including src/dst switch ports in addition to the
	 * switches. This chains into {@link #getPath(DatapathId, DatapathId)} below.
	 * 
	 * @param srcId
	 * @param srcPort
	 * @param dstId
	...
	public Path getPath(DatapathId srcId, DatapathId dstId)
	{
		...
		try {
			...
			log.warn(
				"Could not find route from {} to {}. If the path exists, wait for the topology to settle, and it will be detected",
				srcId, dstId);
		}
	...
	{
		if (isInternalLinkInCluster(newSw, newPort))
			return true;
...
//------------------------------------------------------------------------------
	private int getDijkstraCost(Map<DatapathId, Set<Link>> links,
		DatapathId root, Map<Link, Integer> linkCost,
		boolean isDstRooted)
	{
