...
public class DualAscentTopologyInstance
{
	...
	protected static Logger log = LoggerFactory.getLogger(DualAscentTopologyInstance.class);
...
//------------------------------------------------------------------------------
		...
		Cluster auxiliaryGraph, boolean isDstRooted)
	{
		 ...
		 */
		System.out.println("SIAMO NELLA COSTRUISCI BROADCAST TREE");
		log.info("Auxiliary Graph: {}", auxiliaryGraph.toString());
	...
	private void addArc(Link minLink, Cluster auxiliaryGraph, Cluster completeGraph)
	{
		 ...
		 * links.get(minLink.getSrc()).add(minLink);
		 * links.get(minLink.getDst()).add(minLink);
		 */
		if (minLink != null) {
