 ...
 * {@link DualAscentDualAscentTopologyManager}
 */
...
//------------------------------------------------------------------------------
	...
	protected DualAscentTopologyInstance(Map<DatapathId, Set<OFPort>> portsWithLinks,
		Set<NodePortTuple> portsBlocked, Map<NodePortTuple, Set<Link>> linksNonBcastNonTunnel,
		Set<NodePortTuple> portsWithMoreThanTwoLinks, Set<NodePortTuple> portsTunnel,
		Map<NodePortTuple, Set<Link>> links, Map<DatapathId, Set<OFPort>> portsPerSwitch,
		Map<NodePortTuple, Set<Link>> linksExternal)
	{
		log.info("!! DualAscentTopologyInstance constructor !!");
	...
	public boolean isEdge(DatapathId sw, OFPort portId)
	{
		...
		return links.get(np) == null || links.get(np).isEmpty();
	}
...
//------------------------------------------------------------------------------
		...
		Map<Link, Integer> linkCost, boolean isDstRooted)
	{
		...
		log.info("--- dualAscent() execution complete! ---");
...
//------------------------------------------------------------------------------
	...
	public boolean isAttachmentPointPort(DatapathId switchid, OFPort port)
	{
		...
		return !linksNonBcastNonTunnel.containsKey(npt);
	}
