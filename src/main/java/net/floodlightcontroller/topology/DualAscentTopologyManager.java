 ...
 * the topology.
 */
public class DualAscentTopologyManager implements IFloodlightModule, ITopologyService, ITopologyManagerBackend,
	ILinkDiscoveryListener, IOFMessageListener
{
	private static Logger log = LoggerFactory.getLogger(DualAscentTopologyManager.class);
	public static final String MODULE_NAME = "topology";

	protected static volatile PATH_METRIC pathMetric = PATH_METRIC.HOPCOUNT_AVOID_TUNNELS; // default: compute paths
												// on hop count
	...
	protected DualAscentTopologyInstance currentInstance;
	...
	public Map<DatapathId, Set<Link>> getAllLinks()
	{
		...
		DualAscentTopologyInstance ti = getCurrentInstance();
	...
	public boolean isAttachmentPointPort(DatapathId switchid, OFPort port)
	{
		...
		if (port.equals(OFPort.ALL) || port.equals(OFPort.CONTROLLER) || port.equals(OFPort.ANY) ||
			port.equals(OFPort.FLOOD) || port.equals(OFPort.ZERO) || port.equals(OFPort.NO_MASK) ||
			port.equals(OFPort.IN_PORT) || port.equals(OFPort.NORMAL) || port.equals(OFPort.TABLE))
			return false;
	...
	private class HAListenerDelegate implements IHAListener
	{
		...
		{
			role = HARole.ACTIVE;
			log.debug("Re-computing topology due " + "to HA change from STANDBY->ACTIVE");
			newInstanceTask.reschedule(TOPOLOGY_COMPUTE_INTERVAL_MS, TimeUnit.MILLISECONDS);
		}
		...
		public String getName()
		{
			return DualAscentTopologyManager.this.getName();
		}
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException
	{
		...
		String metric = configOptions.get("pathMetric") != null
			? configOptions.get("pathMetric").trim().toLowerCase()
			: null;
		if (metric != null) {
		...
		String maxroutes = configOptions.get("maxPathsToCompute") != null
			? configOptions.get("maxPathsToCompute").trim()
			: null;
		if (maxroutes != null)
			try {
	...
	protected Command dropFilter(DatapathId sw, OFPacketIn pi, FloodlightContext cntx)
	{
		...
		if (isNotBlocked(sw, inPort) == false)
			if (log.isTraceEnabled()) {
				log.trace("Ignoring packet because of topology " + "restriction on switch={}, port={}",
					sw.getLong(), inPort.getPortNumber());
				result = Command.STOP;
			}
		return result;
	}
	...
	public void doMultiActionPacketOut(byte[] packetData, IOFSwitch sw, Set<OFPort> ports, FloodlightContext cntx)
	{
		...
		// short poLength = (short)(OFPacketOut.MINIMUM_LENGTH + po.getActionsLength() +
		// packetData.length);
	...
	protected void doFloodBDDP(DatapathId pinSwitch, OFPacketIn pi, FloodlightContext cntx)
	{
		...
		for (DatapathId sid: switches) {
			...
			if (pinSwitch == sid)
				ports.remove((pi.getVersion().compareTo(OFVersion.OF_12) < 0 ? pi.getInPort()
					: pi.getMatch().get(MatchField.IN_PORT)));
	...
	protected void addOrUpdateSwitch(DatapathId sw)
	{
		 ...
		 * 
		 * addSwitch(sw); for (OFPortDesc p : switchService.getSwitch(sw).getPorts()) {
	...
	protected boolean createNewInstance(String reason, boolean forced)
	{
		...
		DualAscentTopologyInstance nt = new DualAscentTopologyInstance(switchPorts, blockedPorts, openflowLinks,
			broadcastDomainPorts, tunnelPorts, switchPortLinks, allPorts, interClusterLinks);
	...
	protected Set<NodePortTuple> identifyBroadcastDomainPorts()
	{
		...
		Set<NodePortTuple> additionalNpt = new HashSet<NodePortTuple>();

		// Copy switchPortLinks
		Map<NodePortTuple, Set<Link>> spLinks = new HashMap<NodePortTuple, Set<Link>>();
		...
		for (NodePortTuple npt: spLinks.keySet()) {
			...
			if (links.size() > 2) {
				...
				if (!l1.getSrc().equals(l2.getDst()) || !l1.getSrcPort().equals(l2.getDstPort()) ||
					!l1.getDst().equals(l2.getSrc()) || !l1.getDstPort().equals(l2.getSrcPort()))
					bdPort = true;
			}
	...
	public void removeLink(Link link)
	{
		...
		if (switchPortLinks.get(srcNpt) == null && switchPorts.get(srcNpt.getNodeId()) != null)
				switchPorts.get(srcNpt.getNodeId()).remove(srcNpt.getPortId());
		if (switchPortLinks.get(dstNpt) == null && switchPorts.get(dstNpt.getNodeId()) != null)
			switchPorts.get(dstNpt.getNodeId()).remove(dstNpt.getPortId());

		// Remove the node if no ports are present
		if (switchPorts.get(srcNpt.getNodeId()) != null && switchPorts.get(srcNpt.getNodeId()).isEmpty())
			switchPorts.remove(srcNpt.getNodeId());
		if (switchPorts.get(dstNpt.getNodeId()) != null && switchPorts.get(dstNpt.getNodeId()).isEmpty())
	...
	public DualAscentTopologyInstance getCurrentInstance()
	{
	...
	@Override
	public DualAscentTopologyInstance getCurrentTopologyInstance()
	{
